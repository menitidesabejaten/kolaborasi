document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- 1. DARK MODE TOGGLE (DIOPTIMALKAN) ---
    ...
    themeToggleBtn.addEventListener("click", () => {
        ...
        try {
            ...
            // Fallback for privacy/incognito mode without localStorage
            if (isDark) {
                htmlElement.removeAttribute("data-theme");
                iconTheme.classList.replace("fa-sun", "fa-moon");
            } else {
    ...
    // --- 5. ADMIN LOGIN MODAL LOGIC ---
    const adminBtn = document.getElementById('btn-admin');
    const adminModal = document.getElementById('admin-modal');
    const adminClose = document.getElementById('admin-close');
    const adminCloseMob = document.getElementById('admin-close-mob');
    ...
    const openAdminModal = (e) => {
        ...
        adminError.style.display = 'none'; // reset error view
        adminForm.reset();
    };
    ...
    if (adminCloseMob) adminCloseMob.addEventListener('click', closeAdminModal);
    if (adminModal) {
    ...
    if (adminForm) {
        adminForm.addEventListener('submit', (e) => {
            e.preventDefault(); 
            // Selalu tampilkan error sesuai instruksi
